...
| [**iot-thermo**](./iot-thermo/) | The **IoT starter** — a live fleet thermometer with **your own hardware in the loop**. Raspberry Pis (or `--simulate` on a laptop) run a small Python agent that reports SoC temperature to this Qube over HTTPS — readings land in `env.KV`, a live gauge dashboard polls `/api/fleet` — and, in parallel, the agent speaks the **qubepods node plane**: enroll with a one-time token, then hold the device **trunk** (one *outbound* WebSocket to `/v1/socket`, `hello` + `telemetry` frames, heartbeat poll as the fallback) so the fleet also shows up on the console **Nodes** page — online, geo-located, zero inbound ports. Device identity is claim-on-first-report (a `thermo_…` key returned once, stored hash-at-rest — the platform's own `qnode_` discipline). |
