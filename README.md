...
| [**qube-rocks**](./qube-rocks/) | **Asteroids-lite** — a full game in the **quine** 3D engine, no backend. The whole game loop is a deterministic skill (`onPreStep` at **64 Hz**): steer with `transform.rotation`, fire and split rocks with `world.spawn`/`despawn`, all driven by the **reusable on-screen controller** overlay (`controls.js`) the `water`/`sundial`/`terrain` examples share. Shows the engine's fixed-timestep + render interpolation, runtime spawn/despawn, and the `cone` ship mesh. |
