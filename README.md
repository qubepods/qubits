...
| [**blackbird**](./blackbird/) | A full chess game you play in the browser, shipped as a **static-asset qube**: `qube.json5` declares `static: { dir: "web" }` and `qube deploy` serves the `web/` folder as-is — no q64 compile, no component, no backend. The engine is a **high-performance Rust chess engine compiled to WebAssembly** that runs **in the browser at near-native speed** (no server), loaded by the page as an *asset* wasm — deployed through the same `qube` tooling as a q64 qube. Shows qubepods is **wasm-native and language-agnostic**: bring any wasm, ship it unchanged. |
